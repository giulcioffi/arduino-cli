...
func initRemoveCommand() *cobra.Command {
	addCommand := &cobra.Command{
		Use:   "remove",
		Short: "Removes one or more values from a settings key.",
		Long:  "Removes one or more values from a settings key.",
		Example: "" +
...
func runRemoveCommand(cmd *cobra.Command, args []string) {
	...
	values := configuration.Settings.GetStringSlice(key)
